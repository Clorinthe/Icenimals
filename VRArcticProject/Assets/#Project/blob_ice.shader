    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
				...
				//o.uv0 = v.texcoord0;
				//o.uv0 = float2(v.vertex.x+v.vertex.z, v.vertex.y + v.vertex.z);
				// cartesian 2 spherical > UV use the theta and phi of the spherical system
				float r = sqrt(v.vertex.x*v.vertex.x + v.vertex.y*v.vertex.y + v.vertex.z*v.vertex.z);
				float theta = acos( v.vertex.z / r ) / 3.1415926535898;
				float phi = atan( v.vertex.y / v.vertex.x ) / 3.1415926535898;
				o.uv0 = float2(theta, phi);
                //o.uv1 = v.texcoord1;
                //o.uv2 = v.texcoord2;
				o.uv1 = o.uv0;
				o.uv2 = o.uv0;
                #ifdef LIGHTMAP_ON
                    o.ambientOrLightmapUV.xy = o.uv1.xy * unity_LightmapST.xy + unity_LightmapST.zw;
                    o.ambientOrLightmapUV.zw = 0;
                #elif UNITY_SHOULD_SAMPLE_SH
                #endif
                #ifdef DYNAMICLIGHTMAP_ON
                    o.ambientOrLightmapUV.zw = o.uv2.xy * unity_DynamicLightmapST.xy + unity_DynamicLightmapST.zw;
                #endif
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                o.pos = UnityMetaVertexPosition(v.vertex, o.uv1.xy, o.uv2.xy, unity_LightmapST, unity_DynamicLightmapST );
                return o;
            }
